...
if options.n_channels > 0 :
   fits[0].header['NCHANS'] = options.n_channels
   print("SET NCHANS := %d" % (options.n_channels))
   
if options.n_timesteps > 0 :   
   fits[0].header['NSCANS']  = options.n_timesteps
   print("SET NSCANS := %d" % (options.n_timesteps))
   
if options.inttime > 0 :   
   fits[0].header['INTTIME'] = float( options.inttime )
   print("SET INTTIME := %.4f [sec]" % (float( options.inttime )))
