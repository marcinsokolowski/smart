...
if [[ $max_baseline_int -lt 2800 ]]; then # 20190309 - changed for a proper condition 
    ...
    echo "time $wsclean_path -name wsclean_${ms_b}_briggs -j 6 -size ${imagesize} ${imagesize}  -pol XX,YY,XY,YX -abs-mem 64 -weight briggs -1 -scale $pixscale -nmiter 1 -niter ${n_iter} -local-rms -auto-mask 3 -auto-threshold 1  -mgain 0.5 -minuv-l 30 ${options} ${ms}"
    time $wsclean_path -name wsclean_${ms_b}_briggs -j 6 -size ${imagesize} ${imagesize}  -pol XX,YY,XY,YX -abs-mem 64 -weight briggs -1 -scale $pixscale -nmiter 1 -niter ${n_iter} -local-rms -auto-mask 3 -auto-threshold 1  -mgain 0.5 -minuv-l 30 ${options} ${ms}
else
