...
#            peak_flux=float(words[4+0])
            ...
            sources.append( ImageSource(-1,-1,ra,dec,-1) )
      file.close()
...
if __name__ == '__main__':
   ...
   print("List of sources = %s" % (options.source_list_file))
   ...
   source_list = []
   if options.source_list_file is not None :
      source_list = read_sources( options.source_list_file )
   else :
      source_list.append( ImageSource(-1,-1,options.ra,options.dec,-1) )
   ...
   for fitsfile_bytes in fitslist_data :
      ...
      for source in source_list :
         ra = options.ra
         dec = options.dec
         print("PROGRESS : dumping lightcurve for source (RA,DEC) = (%.4f,%.4f) [deg]" % (ra,dec))
...
#            coord.obstime = Time("20200507T065634", scale='utc', format="utc" )
         ...
         (x_c0,y_c0) = sky2pix.sky2pix( fits, ra, dec )
...
#   print("Read FITS file %s has dateobs = %s -> %.2f unixtime , (%.4f,%.4f) [deg] -> (%.2f,%.2f) [pixels in ds9 convention] -> (%.2f,%.2f) [pixels in python/C convention]" % (fitsfile,dateobs,t_unix.value,options.ra,options.dec,x_c0,y_c0,x_c0_new,y_c0_new))
         print("Read FITS file %s has dateobs = %s -> %.2f unixtime , (%.4f,%.4f) [deg] -> (%.2f,%.2f) [pixels in python/C indexing convention]" % (fitsfile,dateobs,t_unix.value,ra,dec,x_c0,y_c0))
...
#   y_c = int( numpy.round(y_c0) )
         ...
         if not numpy.isnan(x_c0) and not numpy.isnan(y_c0) :
            ...
            print("WARNING : (RA,DEC) = (%.4f,%.4f) converted to (x,y) gives at least one NaN value -> %s skipped" % (ra, dec, fitsfile))
            continue
